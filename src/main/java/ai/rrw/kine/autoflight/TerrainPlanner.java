...
public final class TerrainPlanner {
    ...
    static final int CLEARS = 1, HITS = 0, UNKNOWN = -1;
    ...
    static int rollout(FlightModel.State base, double target, Terrain terrain,
                       int horizon, double buffer) {
        FlightModel.State s = base.copy();
        s.target = target;
        for (int i = 0; i < horizon; i++) {
            FlightModel.step(s);
            double g = terrain.heightAt(s.x);
            if (Double.isNaN(g)) return UNKNOWN;          // ran into unknown terrain in horizon
    ...
    public static double plan(FlightModel.State base, Terrain terrain,
        ...
        for (int k = 0; k <= n; k++) {
            ...
            if (rollout(base, tgt, terrain, horizon, buffer) == CLEARS) return tgt;
        }
        return DISENGAGE;
    }
}
